...
#                status = MIFAREReader.MFRC522_Auth(MIFAREReader.PICC_AUTHENT1A, 8, key, uid)
                status = MIFAREReader.MFRC522_Auth(MIFAREReader.PICC_AUTHENT1A, 48, key, uid)

                # Check if authenticated.
                if status == MIFAREReader.MI_OK:
                    MIFAREReader.MFRC522_Read(48)
                    MIFAREReader.MFRC522_StopCrypto1()
                    sys.exit("[!] SUCCESS [!] - The correct key is " + str(key))

                else:
                    print("[-] Authentication error, closing SPI")
                    MIFAREReader.MFRC522_CloseSPI()
                    continue_reading = False
        #else:
